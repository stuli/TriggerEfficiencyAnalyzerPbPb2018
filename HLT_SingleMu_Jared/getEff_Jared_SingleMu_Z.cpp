...
//main
void getEff_Jared_SingleMu_Z(TString tagString = "HLT_HIL3Mu3_v1", int kTagPath=1){
...
*/
  fcha->Add("/eos/cms/store/group/phys_heavyions/dileptons/pshukla/OniaTrigger18/Zm10m10_OniaForest.root");//PromptReco_Double_All(8TeV)
...
*/
  ...
  for(int i=0; i<nevent; i++){
    fcha->GetEntry(i);
    if(i%2000==0){cout<<">>>>> EVENT "<<i<<" / "<<fcha->GetEntries()<<" ("<<(int)(100.*i/fcha->GetEntries())<<"%)"<<endl;}
...
//  leg->AddEntry(grp_SQ[0],"L1DoubleMu0_SQ","p");
  ...
  drawText2("Z boson",1.5,1.2,17);
  dashedLine(0,1,15,1,1);

  c0->SaveAs("HLT_SingleMu_Z_pt.pdf");
  c0->SaveAs("HLT_SingleMu_Z_pt.png");
...
//  lege->AddEntry(gre_SQ[0],"L1DoubleMu0_SQ","p");
  ...
  drawText2("Z boson",-1.0, 1.2,17);
  dashedLine(-2.4,1,2.4,1,1);

  c1->SaveAs("HLT_SingleMu_Z_eta.pdf");
  c1->SaveAs("HLT_SingleMu_Z_eta.png");

  //Save the Graphs in a root file
  TFile* outFile = new TFile("HLT_SingleMu_Z.root","recreate");
  c0->Write();
  c1->Write();
  for(int k=kStart;k<Ntrig;k++){
