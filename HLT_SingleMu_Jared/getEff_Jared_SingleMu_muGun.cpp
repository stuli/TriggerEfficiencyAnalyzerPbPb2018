...
*/
  fcha->Add("/eos/cms/store/group/phys_heavyions/dileptons/pshukla/OniaTrigger18/SingleMuPt_0_30_OniaForest.root");
...
//  leg->AddEntry(grp_SQ[0],"L1DoubleMu0_SQ","p");
  ...
  drawText2("SingleMu 0<p_{T}<30GeV/c",1.5,1.2,17);
  dashedLine(0,1,15,1,1);

  c0->SaveAs("HLT_SingleMu_muGun_pt.pdf");
  c0->SaveAs("HLT_SingleMu_muGun_pt.png");
...
//  lege->AddEntry(gre_SQ[0],"L1DoubleMu0_SQ","p");
  ...
  drawText2("muGun 0<p_{T}<30GeV/c",-1.0, 1.2,17);
  dashedLine(-2.4,1,2.4,1,1);

  c1->SaveAs("HLT_SingleMu_muGun_eta.pdf");
  c1->SaveAs("HLT_SingleMu_muGun_eta.png");

  //Save the Graphs in a root file
  TFile* outFile = new TFile("HLT_SingleMu_muGun.root","recreate");
  c0->Write();
  c1->Write();
  for(int k=kStart;k<Ntrig;k++){
