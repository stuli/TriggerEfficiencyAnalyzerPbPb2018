...
//main
void getEff_Jared_SingleMu(TString tagString = "HLT_HIL3Mu3_v1", int kTagPath=1){
...
*/
 ...
 //define Trigger
    ...
    for(int j=0; j<Reco_mu_size; j++){
      ...
      for(int k=kStart; k<Ntrig; k++){
          ...
          )
        {
          ...
          deno_e[k]->Fill(recomu4mom->Eta());//denominator for SingleMu trigs
  ...
  grp[kStart]->GetXaxis()->SetLimits(0,40);
  grp[kStart]->GetXaxis()->SetRangeUser(0,40);
...
//  leg->AddEntry(grp_SQ[0],"L1DoubleMu0_SQ","p");
  ...
  dashedLine(0,1,40,1,1);

  c0->SaveAs("HLT_SingleMu_pt.pdf");
...
//  lege->AddEntry(gre_SQ[0],"L1DoubleMu0_SQ","p");
  ...
  drawText2("SingleMu 0<p_{T}<30GeV/c",-1.0, 1.2,17);
  dashedLine(-2.4,1,2.4,1,1);

  c1->SaveAs("HLT_SingleMu_eta.pdf");
