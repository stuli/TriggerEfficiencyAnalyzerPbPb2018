...
//main
void getEff_Jared_SingleMu_Ups1S_0_30(TString tagString = "HLT_HIL3Mu3_v1", int kTagPath=1){
...
*/
  fcha->Add("/eos/cms/store/group/phys_heavyions/dileptons/pshukla/OniaTrigger18/Ups1SMM_0_30_OniaForest1.root");
  fcha->Add("/eos/cms/store/group/phys_heavyions/dileptons/pshukla/OniaTrigger18/Ups1SMM_0_30_OniaForest2.root");
  fcha->Add("/eos/cms/store/group/phys_heavyions/dileptons/pshukla/OniaTrigger18/Ups1SMM_0_30_OniaForest3.root");
  fcha->Add("/eos/cms/store/group/phys_heavyions/dileptons/pshukla/OniaTrigger18/Ups1SMM_0_30_OniaForest4.root");
...
*/
  ...
  //Int_t nevent_hlt = fcha_hltBit->GetEntries();
  //Int_t nevent = 100000;
  //Int_t nevent_hlt = 100000;

  //if(nevent != nevent_hlt){
    //cout << "can't " << endl; 
    //return;
  //}
   
  TH2D* h_hlt_onia = new TH2D("h_hlt_onia",";Reco_mu_size;NL1Stage2Muon",10,0,10,10,0,10);

  for(int i=0; i<nevent; i++){
    fcha->GetEntry(i);
    if(i%20000==0){cout<<">>>>> EVENT "<<i<<" / "<<fcha->GetEntries()<<" ("<<(int)(100.*i/fcha->GetEntries())<<"%)"<<endl;}
    
    //fcha_hltBit->GetEntry(i);  

    //h_hlt_onia->Fill(Reco_mu_size,NL1Stage2Muon); 
    //if((Reco_mu_size >1) || (NL1Stage2Muon>1)) continue; 
  ...
  /*TGraphAsymmErrors *grp_OS[10];
  TGraphAsymmErrors *gre_OS[10];
  TGraphAsymmErrors *grp_SQ[10];
  TGraphAsymmErrors *gre_SQ[10];
  TGraphAsymmErrors *grp_SQOS[10];
  TGraphAsymmErrors *gre_SQOS[10];
  TGraphAsymmErrors *grp_mass[10];
  TGraphAsymmErrors *gre_mass[10];
  TGraphAsymmErrors *grp_dR[10];
  TGraphAsymmErrors *gre_dR[10];
  TGraphAsymmErrors *grp_open[10];
  TGraphAsymmErrors *gre_open[10];*/
  for(int k=kStart;k<Ntrig;k++){
    nume_p[k]->Divide(nume_p[k],deno_p[k],1,1,"B");
    nume_e[k]->Divide(nume_e[k],deno_e[k],1,1,"B");
    //nume_p_l1SQ[k]->Divide(nume_p_l1SQ[k],deno_p_l1SQ[k],1,1,"B");
    //nume_e_l1SQ[k]->Divide(nume_e_l1SQ[k],deno_e_l1SQ[k],1,1,"B");
    //nume_p_l1SQOS[k]->Divide(nume_p_l1SQOS[k],deno_p_l1SQOS[k],1,1,"B");
    //nume_e_l1SQOS[k]->Divide(nume_e_l1SQOS[k],deno_e_l1SQOS[k],1,1,"B");
    //nume_p_l1OS[k]->Divide(nume_p_l1OS[k],deno_p_l1OS[k],1,1,"B");
    //nume_e_l1OS[k]->Divide(nume_e_l1OS[k],deno_e_l1OS[k],1,1,"B");
    //nume_p_l1mass[k]->Divide(nume_p_l1mass[k],deno_p_l1mass[k],1,1,"B");
    //nume_e_l1mass[k]->Divide(nume_e_l1mass[k],deno_e_l1mass[k],1,1,"B");
    //nume_p_l1dR[k]->Divide(nume_p_l1dR[k],deno_p_l1dR[k],1,1,"B");
    //nume_e_l1dR[k]->Divide(nume_e_l1dR[k],deno_e_l1dR[k],1,1,"B");
    //nume_p_l1open[k]->Divide(nume_p_l1open[k],deno_p_l1open[k],1,1,"B");
    //nume_e_l1open[k]->Divide(nume_e_l1open[k],deno_e_l1open[k],1,1,"B");
    grp[k] = new TGraphAsymmErrors(nume_p[k]);
    gre[k] = new TGraphAsymmErrors(nume_e[k]);
    //grp_SQ[k] = new TGraphAsymmErrors(nume_p_l1SQ[k]);
    //gre_SQ[k] = new TGraphAsymmErrors(nume_e_l1SQ[k]);
    //grp_SQOS[k] = new TGraphAsymmErrors(nume_p_l1SQOS[k]);
    //gre_SQOS[k] = new TGraphAsymmErrors(nume_e_l1SQOS[k]);
    //grp_OS[k] = new TGraphAsymmErrors(nume_p_l1OS[k]);
    //gre_OS[k] = new TGraphAsymmErrors(nume_e_l1OS[k]);
    //grp_mass[k] = new TGraphAsymmErrors(nume_p_l1mass[k]);
    //gre_mass[k] = new TGraphAsymmErrors(nume_e_l1mass[k]);
    //grp_dR[k] = new TGraphAsymmErrors(nume_p_l1dR[k]);
    //gre_dR[k] = new TGraphAsymmErrors(nume_e_l1dR[k]);
    //grp_open[k] = new TGraphAsymmErrors(nume_p_l1open[k]);
    //gre_open[k] = new TGraphAsymmErrors(nume_e_l1open[k]);

    //grp[k] = new TGraphAsymmErrors(nume_p[k],deno_p[k],"pois");
    //gre[k] = new TGraphAsymmErrors(nume_e[k],deno_p[k],"pois");
    ...
    /*if(k==kStart){
      SetGraphStyle(grp_dR[k],1,1);
      SetGraphStyle(gre_dR[k],1,1);
      //  SetGraphStyle(grp_SQOS[k],2,2);
      //  SetGraphStyle(gre_SQOS[k],2,2);
      //SetGraphStyle(grp_open[k],2,2);
      //SetGraphStyle(gre_open[k],2,2);
      //SetGraphStyle(grp_mass[k],3,3);
      //SetGraphStyle(gre_mass[k],3,3);
      //SetGraphStyle(grp_SQ[k],4,4);
      //SetGraphStyle(gre_SQ[k],4,4);
      //SetGraphStyle(grp_OS[k],5,5);
      //SetGraphStyle(gre_OS[k],5,5);
    }*/
  }
  ...
  TLegend* lege = new TLegend(0.6,0.3,0.8,0.5);
  SetLegendStyle(lege);
  lege->SetTextSize(0.03);

  TCanvas* c0 = new TCanvas("c0","",600,600);
  c0->cd();
  grp[kStart]->GetYaxis()->SetRangeUser(0,1.3);
  grp[kStart]->GetXaxis()->SetLimits(0,15);
  grp[kStart]->GetXaxis()->SetRangeUser(0,15);
  grp[kStart]->GetXaxis()->SetTitle("p^{#mu}_{T} (GeV/c))");
  grp[kStart]->GetYaxis()->SetTitle("Efficiency");
  grp[kStart]->GetYaxis()->CenterTitle();
  grp[kStart]->GetXaxis()->CenterTitle();
  grp[kStart]->Draw("ap");
//  grp_dR[0]->Draw("p same");
//  grp_open[0]->Draw("p same");
//  grp_mass[0]->Draw("p same");
//  grp_SQ[0]->Draw("p same");
//  grp_OS[0]->Draw("p same");
  for(int k=kStart+1;k<Ntrig;k++){
    grp[k]->Draw("p same");
  }

  for(int k=kStart;k<Ntrig;k++){
    TString legname = trigname[k];
    leg->AddEntry(grp[k],legname,"p");
  }  

//  leg->AddEntry(grp[kStart],"L1SingleMuOpen","p");
//  leg->AddEntry(grp_dR[0],"L1DoubleMu+dR>0.3","p");
//  leg->AddEntry(grp_open[0],"L1DoubleMuOpen","p");
//  leg->AddEntry(grp_mass[0],"L1DoubleMu0+mass>3GeV","p");
//  leg->AddEntry(grp_SQ[0],"L1DoubleMu0_SQ","p");
 
  leg->Draw("same");
  
  drawText2("#Upsilon(1S) 0<p_{T}<30GeV/c",1.5,1.2,17);
  dashedLine(0,1,15,1,1);

  c0->SaveAs("HLT_SingleMu_Ups1S_0_30_pt.pdf");
  c0->SaveAs("HLT_SingleMu_Ups1S_0_30_pt.png");

  TCanvas* c1 = new TCanvas("c1","",600,600);
  c1->cd();
  gre[kStart]->GetYaxis()->SetRangeUser(0,1.3);
  gre[kStart]->GetXaxis()->SetRangeUser(-2.4,2.4);
  gre[kStart]->GetXaxis()->SetLimits(-2.4,2.4);
  gre[kStart]->GetXaxis()->SetTitle("#eta^{#mu}");
  gre[kStart]->GetYaxis()->SetTitle("Efficiency");
  gre[kStart]->GetYaxis()->CenterTitle();
  gre[kStart]->GetXaxis()->CenterTitle();
  gre[kStart]->Draw("ap");
//  gre_dR[0]->Draw("p same");
//  gre_open[0]->Draw("p same");
//  gre_mass[0]->Draw("p same");
//  gre_SQ[0]->Draw("p same");
//  gre_OS[0]->Draw("p same");
  for(int k=kStart+1;k<Ntrig;k++){
    gre[k]->Draw("p same");
  }

  for(int k=kStart;k<Ntrig;k++){
    TString legename = trigname[k];
    lege->AddEntry(gre[k],legename,"p");
  }

//  lege->AddEntry(gre[0],"L1SingleMuOpen","p");
//  lege->AddEntry(gre_dR[0],"L1DoubleMu+dR>0.3","p");
//  lege->AddEntry(gre_open[0],"L1DoubleMuOpen","p");
//  lege->AddEntry(gre_mass[0],"L1DoubleMu0+mass>3GeV","p");
//  lege->AddEntry(gre_SQ[0],"L1DoubleMu0_SQ","p");
 
  lege->Draw("same");
  
  drawText2("#Upsilon(1S) 0<p_{T}<30GeV/c",-1.0, 1.2,17);
  dashedLine(-2.4,1,2.4,1,1);

  c1->SaveAs("HLT_SingleMu_Ups1S_0_30_eta.pdf");
  c1->SaveAs("HLT_SingleMu_Ups1S_0_30_eta.png");

  //Save the Graphs in a root file
  TFile* outFile = new TFile("HLT_SingleMu_Ups1S_0_30.root","recreate");
  c0->Write();
  c1->Write();
  for(int k=kStart;k<Ntrig;k++){
